...
2. Navigate to https://serverless-1.us-east-2.s.union.ai/
...
Run the folloining to get the execution id of the workflow we just launched.
...
llm_executions = [
    e for e in recent_executions if e.spec.launch_plan.name == "llm_tracking.main"
]

EXECUTION_ID = llm_executions[0].id.name
...
In the VSCode environment open the `interact_with_model.py` file and click on `Run Cell` button at the top of the file. We will highlight how to get the model in Union's VSCode enviornment and how to use Jupyter to interact query the model.
